...
func (a *App) Routes() []p.Route {
	return []p.Route{
		{Path: "/", HTMLFile: "index.html", SSRPath: "/", Component: &App{}},
	}
}
...
go run github.com/tbocek/preveltekit/v2/cmd/build@latest init</code></pre>
				...
				<div class="step">
					...
					<div class="step-content">
						<h3>Write your component in Go</h3>
						<pre><code>type Hello struct {
    Name *p.Store[string]
}

func (h *Hello) New() p.Component {
    return &Hello{Name: p.New("World")}
}

func (h *Hello) Render() p.Node {
    return p.Html(` + "`" + `&lt;h1>Hello, ` + "`" + `, h.Name, ` + "`" + `!&lt;/h1>` + "`" + `)
}</code></pre>
					</div>
				</div>
				<div class="step">
					...
					<div class="step-content">
						...
						<pre><code>./build.sh --release</code></pre>
						<p class="step-hint">Output goes to <code>dist/</code> — pure static files ready to deploy.</p>
					</div>
				</div>
			</div>
		</div>
...
.feature-card{padding:24px;border:1px solid #e5e7eb;border-radius:8px;text-align:center}
...
.quickstart{padding:60px 0;background:#f8f9fa}
.quickstart h2{text-align:center;font-size:2em;margin-bottom:40px;color:#1a1a2e}
...
.highlight-grid,.feature-grid{grid-template-columns:1fr}
