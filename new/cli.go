...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("usage: pregokit <file.web.go>")
		os.Exit(1)
	}
	...
	// Merge reactive vars from parser into analysis
	for varName, varType := range p.ReactiveVars {
		if _, exists := a.Vars[varName]; !exists {
			a.Vars[varName] = &Variable{
				Name:      varName,
				Type:      varType,
				DependsOn: []string{},
				Affects:   []string{},
			}
			a.Order = append(a.Order, varName)
		} else {
			a.Vars[varName].Type = varType
		}
	}
	...
	// Copy local .go files to dist (excluding the source file)
	...
	for name := range p.LocalGoFiles {
		fmt.Printf("  %s\n", name)
	}
	fmt.Printf("\nReactive vars: ")
	for name := range p.ReactiveVars {
		fmt.Printf("%s ", name)
	}
	fmt.Println()
	fmt.Printf("\nBuild: cd %s && tinygo build -o app.wasm -target wasm -no-debug -panic trap -scheduler none -gc leaking .\n", distDir)
}